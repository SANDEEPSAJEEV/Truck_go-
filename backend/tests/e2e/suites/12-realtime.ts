...
import { ctx, createBooking, goOnline, KOCHI_NEARBY, liveToken } from "../actors";
...
suite("realtime", "12 — Realtime", () => {
  ...
  test("12.1", "a valid access token connects", async () => {
    const socket = await TestSocket.connect("valid", await liveToken(ctx.rider));
    socket.close();
  });
  ...
  test("12.6", "every socket auto-joins its own user room", async () => {
    ...
    const socket = await TestSocket.connect("driverA-userroom", await liveToken(ctx.driverA));
    try {
  ...
  test("12.7", "subscribing to a trip delivers its status changes", async () => {
    ...
    const socket = await TestSocket.connect("rider-trip", await liveToken(ctx.rider));
    try {
  ...
  test("12.8", "unsubscribing stops the delivery", async () => {
    const booking = await createBooking(ctx.rider);
    const socket = await TestSocket.connect("rider-unsub", await liveToken(ctx.rider));
    try {
      await subscribeTrip(socket, booking.id);
  ...
  test("12.9", "a driver's position relays to the rider, and not back to the driver", async () => {
    ...
    const rider = await TestSocket.connect("rider-loc", await liveToken(ctx.rider));
    const driver = await TestSocket.connect("driver-loc", await liveToken(ctx.driverA));
    try {
  ...
  test("12.10", "bid:new reaches the rider and nobody else", async () => {
    const booking = await createBooking(ctx.rider);
    const rider = await TestSocket.connect("rider-bidnew", await liveToken(ctx.rider));
    const otherRider = await TestSocket.connect("rider2-bidnew", await liveToken(ctx.rider2));
    try {
  ...
  test("12.11", "accepting routes bid:accepted to the winner and bid:rejected to the loser", async () => {
    ...
    const winner = await TestSocket.connect("winner", await liveToken(ctx.driverA));
    const loser = await TestSocket.connect("loser", await liveToken(ctx.driverB));
    try {
  ...
  test("12.12", "load:taken must not be broadcast to every connected socket", async () => {
    await goOnline(ctx.driverA, KOCHI_NEARBY);
    const booking = await createBooking(ctx.rider);
    const placed = await api(`/bookings/${booking.id}/bids`, {
      method: "POST",
    ...
    const bystander = await TestSocket.connect("bystander", await liveToken(ctx.rider2));
    try {
      await api(`/bookings/${booking.id}/bids/${placed.body.bid.id}/accept`, {
        method: "POST",
        token: ctx.rider.accessToken,
  ...
  test("12.13", "a stranger must not be able to join someone else's trip room", async () => {
    const booking = await createBooking(ctx.rider);
    const placed = await api(`/bookings/${booking.id}/bids`, {
      method: "POST",
      token: ctx.driverA.accessToken,
    ...
    const intruder = await TestSocket.connect("intruder", await liveToken(ctx.rider2));
    const driver = await TestSocket.connect("driver-gps", await liveToken(ctx.driverA));
    try {
  ...
  test("12.14", "presence:location from a socket with no part in the trip must be dropped", async () => {
    const booking = await createBooking(ctx.rider);
    const placed = await api(`/bookings/${booking.id}/bids`, {
      method: "POST",
      token: ctx.driverA.accessToken,
    ...
    const rider = await TestSocket.connect("rider-forge", await liveToken(ctx.rider));
    const forger = await TestSocket.connect("forger", await liveToken(ctx.rider2));
    try {
      await subscribeTrip(rider, booking.id);
  ...
  test("12.15", "chat:send from an unrelated socket must be dropped", async () => {
    const booking = await createBooking(ctx.rider);
    const rider = await TestSocket.connect("rider-chat", await liveToken(ctx.rider));
    const outsider = await TestSocket.connect("outsider-chat", await liveToken(ctx.rider2));
    try {
      await subscribeTrip(rider, booking.id);
  ...
  test("12.16", "payment:update reaches both parties on the trip", async () => {
    ...
    const rider = await TestSocket.connect("rider-pay-room", await liveToken(ctx.rider));
    try {
